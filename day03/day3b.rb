class DeliveryRoute
  ...
  def execute(route)
    santa_step("")
    robot_step("")
    route.split("").each_with_index do |c, i|
      if i % 2 == 0
        santa_step(c)
      else
        robot_step(c)
  ...
  def santa_step(step_type)
    case step_type
    when '^' then @santay -= 1
    when 'v' then @santay += 1
    when '>' then @santax += 1
    when '<' then @santax -= 1
    end
    @counts["#{@santax},#{@santay}"] += 1
  end

  def robot_step(step_type)
    case step_type
    when '^' then @roboty -= 1
    when 'v' then @roboty += 1
    when '>' then @robotx += 1
    when '<' then @robotx -= 1
    end
    @counts["#{@robotx},#{@roboty}"] += 1
  end
...
begin
  test("#1", DeliveryRoute.new.execute("^v"), 3)
  test("#2", DeliveryRoute.new.execute("^>v<"), 3)
  test("#3", DeliveryRoute.new.execute("^v^v^v^v^v"), 11)
